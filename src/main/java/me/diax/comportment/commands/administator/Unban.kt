        ...
        message.guild.controller.bans.queue { bans ->
            message.mentionedUsers.map { user ->
                ...
                if (bans.contains(user)) {
                    message.channel.sendMessage(MessageUtil.errorEmbed("$user is not banned.")).queue()
                }
                message.guild.controller.unban(user).queue{_ -> message.channel.sendMessage(MessageUtil.basicEmbed("$user has been unbanned.")).queue()}
            }
        }
    }
}